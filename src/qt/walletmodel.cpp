...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered tokugawa address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TOK.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal tokugawa:URI (tokugawa:XyZ...?message=example)
            {
...
static void NotifyzTOKReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzTOKReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzTOKReset.connect(boost::bind(NotifyzTOKReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzTOKReset.disconnect(boost::bind(NotifyzTOKReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->ztokTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
