...
#include "ui_ztokcontroldialog.h"
...
std::set<std::string> ZTokControlDialog::setSelectedMints;
std::set<CMintMeta> ZTokControlDialog::setMints;

ZTokControlDialog::ZTokControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZTokControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZTokControlDialog::~ZTokControlDialog()
{
    delete ui;
}

void ZTokControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTokControlDialog::updateList()
{
...
void ZTokControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTokControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZTok_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTokControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZTokControlDialog::GetSelectedMints()
{
...
void ZTokControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
