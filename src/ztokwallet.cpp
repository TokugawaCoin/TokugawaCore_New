...
CzTOKWallet::CzTOKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing ztok seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZTOKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZTOKSeed_deprecated()) {
                    LogPrintf("%s: Updated zTOK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old ztok seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of ztok wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzTOKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZTOKCount(nCountLastUsed);
    else if (!walletdb.ReadZTOKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzTOKWallet::Lock()
{
    seedMaster = 0;
}

void CzTOKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzTOKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZTOK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzTOKWallet::LoadMintPoolFromDB()
{
...
void CzTOKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzTOKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzTOKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->ztokTracker->HasPubcoinHash(pMint.first)) {
...
bool CzTOKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZTOK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to ztokTracker which also adds to database
    pwalletMain->ztokTracker->Add(dMint, true);
...
void CzTOKWallet::SeedToZTOK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzTOKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzTOKWallet::GenerateDeterministicZTOK(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzTOKWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzTOKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
