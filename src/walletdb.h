...
public:
    ...
    bool WriteZTOKSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZTOKSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZTOKSeed_deprecated(uint256& seed);
    bool EraseZTOKSeed();
    bool EraseZTOKSeed_deprecated();

    bool WriteZTOKCount(const uint32_t& nCount);
    bool ReadZTOKCount(uint32_t& nCount);
